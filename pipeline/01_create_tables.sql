...
-- PK/FK are informational only in Unity Catalog - NOT NULL and CHECK do the real enforcement.
-- RELY lets the optimiser trust the keys, which holds because 02_bronze_to_silver dedupes first.
-- Gold dimensions use deterministic sha2 hash surrogate keys, so a rebuild reproduces identical
-- keys and the facts can be rebuilt independently - see 03_silver_to_gold.
--
-- Duplicates: geolocation is the only source with true duplicates (261,831 identical rows) and is
-- aggregated to one row per prefix. Repeated review_ids and customer_unique_ids are not
-- duplicates - both are resolved by the grains below. Every table is deduplicated on its key
-- before writing.
--
-- Created parents-first so the FK references resolve. IF NOT EXISTS means schema changes need a
-- DROP SCHEMA ... CASCADE first.

USE CATALOG IDENTIFIER(:catalog);

-- COMMAND ----------

-- Conformed from customers, sellers and geolocation: city and state from the first two (whose
-- values are clean), coordinates from geolocation. Only prefixes a customer or seller actually
-- references, so every FK resolves; coordinates are null for the 285 missing from geolocation.
CREATE TABLE IF NOT EXISTS silver.geography (
...
CREATE TABLE IF NOT EXISTS silver.customers (
  ...
  CONSTRAINT customers_geography_fk FOREIGN KEY (zip_code_prefix) REFERENCES silver.geography
)
...
CREATE TABLE IF NOT EXISTS silver.sellers (
  ...
  CONSTRAINT sellers_geography_fk FOREIGN KEY (zip_code_prefix) REFERENCES silver.geography
)
...
CREATE TABLE IF NOT EXISTS silver.products (
  ...
  CONSTRAINT products_category_fk FOREIGN KEY (category_name) REFERENCES silver.product_categories
)
...
CREATE TABLE IF NOT EXISTS silver.orders (
  ...
  CONSTRAINT orders_customers_fk FOREIGN KEY (customer_id) REFERENCES silver.customers
)
...
CREATE TABLE IF NOT EXISTS silver.order_items (
  ...
  CONSTRAINT order_items_orders_fk FOREIGN KEY (order_id) REFERENCES silver.orders,
  CONSTRAINT order_items_products_fk FOREIGN KEY (product_id) REFERENCES silver.products,
  CONSTRAINT order_items_sellers_fk FOREIGN KEY (seller_id) REFERENCES silver.sellers
)
CLUSTER BY (order_id)
...
CREATE TABLE IF NOT EXISTS silver.order_payments (
  ...
  CONSTRAINT order_payments_orders_fk FOREIGN KEY (order_id) REFERENCES silver.orders
)
...
CREATE TABLE IF NOT EXISTS silver.order_reviews (
  ...
  CONSTRAINT order_reviews_reviews_fk FOREIGN KEY (review_id) REFERENCES silver.reviews,
  CONSTRAINT order_reviews_orders_fk FOREIGN KEY (order_id) REFERENCES silver.orders
)
COMMENT 'Links reviews to the orders they cover';

-- COMMAND ----------

-- Gold: star schema, denormalised from silver for analytics. Two facts because the data has two
-- natural grains - see 03_silver_to_gold for the full justification.
...
-- The headline denormalisation: silver keys customers per order, this is one row per real person.
...
CREATE TABLE IF NOT EXISTS gold.fact_order (
  ...
  CONSTRAINT fact_order_customer_fk FOREIGN KEY (customer_key) REFERENCES gold.dim_customer,
  CONSTRAINT fact_order_date_fk FOREIGN KEY (purchase_date_key) REFERENCES gold.dim_date
)
...
CREATE TABLE IF NOT EXISTS gold.fact_order_item (
  ...
  freight_value     DECIMAL(12,2) NOT NULL COMMENT 'Olist apportions freight across an order''s items, so this sums correctly',
  ...
  CONSTRAINT fact_order_item_customer_fk FOREIGN KEY (customer_key) REFERENCES gold.dim_customer,
  CONSTRAINT fact_order_item_product_fk FOREIGN KEY (product_key) REFERENCES gold.dim_product,
  CONSTRAINT fact_order_item_seller_fk FOREIGN KEY (seller_key) REFERENCES gold.dim_seller,
  CONSTRAINT fact_order_item_date_fk FOREIGN KEY (purchase_date_key) REFERENCES gold.dim_date
)
CLUSTER BY (purchase_date_key)
...
-- CHECK constraints can only be added after the table exists, so they live here rather than
-- inline. DROP ... IF EXISTS first keeps this cell re-runnable.

ALTER TABLE silver.products DROP CONSTRAINT IF EXISTS products_dimensions_non_negative;
ALTER TABLE silver.products ADD CONSTRAINT products_dimensions_non_negative CHECK (
...
ALTER TABLE silver.orders DROP CONSTRAINT IF EXISTS orders_status_known;
ALTER TABLE silver.orders ADD CONSTRAINT orders_status_known CHECK (
  order_status IN ('created', 'approved', 'invoiced', 'processing',
                   'shipped', 'delivered', 'unavailable', 'canceled')
);

ALTER TABLE silver.order_items DROP CONSTRAINT IF EXISTS order_items_amounts_non_negative;
ALTER TABLE silver.order_items ADD CONSTRAINT order_items_amounts_non_negative
  CHECK (price >= 0 AND freight_value >= 0);

ALTER TABLE silver.order_items DROP CONSTRAINT IF EXISTS order_items_sequence_positive;
ALTER TABLE silver.order_items ADD CONSTRAINT order_items_sequence_positive
  CHECK (order_item_id >= 1);

ALTER TABLE silver.order_payments DROP CONSTRAINT IF EXISTS order_payments_amounts_non_negative;
ALTER TABLE silver.order_payments ADD CONSTRAINT order_payments_amounts_non_negative
  CHECK (payment_value >= 0 AND coalesce(payment_installments, 0) >= 0);

ALTER TABLE silver.order_payments DROP CONSTRAINT IF EXISTS order_payments_sequence_positive;
ALTER TABLE silver.order_payments ADD CONSTRAINT order_payments_sequence_positive
  CHECK (payment_sequential >= 1);

ALTER TABLE silver.reviews DROP CONSTRAINT IF EXISTS reviews_score_in_range;
ALTER TABLE silver.reviews ADD CONSTRAINT reviews_score_in_range
  CHECK (review_score BETWEEN 1 AND 5);

-- COMMAND ----------

ALTER TABLE gold.fact_order DROP CONSTRAINT IF EXISTS fact_order_amounts_non_negative;
ALTER TABLE gold.fact_order ADD CONSTRAINT fact_order_amounts_non_negative
  CHECK (item_value >= 0 AND freight_value >= 0 AND order_value >= 0);

ALTER TABLE gold.fact_order DROP CONSTRAINT IF EXISTS fact_order_review_score_in_range;
ALTER TABLE gold.fact_order ADD CONSTRAINT fact_order_review_score_in_range
  CHECK (review_score IS NULL OR review_score BETWEEN 1 AND 5);

ALTER TABLE gold.fact_order_item DROP CONSTRAINT IF EXISTS fact_order_item_amounts_non_negative;
ALTER TABLE gold.fact_order_item ADD CONSTRAINT fact_order_item_amounts_non_negative
  CHECK (item_value >= 0 AND freight_value >= 0);

ALTER TABLE gold.dim_customer DROP CONSTRAINT IF EXISTS dim_customer_lifetime_non_negative;
ALTER TABLE gold.dim_customer ADD CONSTRAINT dim_customer_lifetime_non_negative
